...
class GeneralEO(EOStrategy, BaseModel):
    ...
    prefer_axis_diversity: bool = Field(
        default=False, description="Try to check same number of DRs on each axis if possible"
    )
    ...
    def select_eos(self, eos: List[Step]) -> List[Step]:
        ...
        eos_by_axis = list(itertools.zip_longest(*(sorted(list(i), key=self.sort_order) for _,i in itertools.groupby(eos, lambda s: s.name))))
        eos = []
...
class OptimalDR(DRStrategy, BaseModel):
    ...
    def select_drs(self, drs: List[Step]) -> List[Step]:
        ...
        drs_by_axis = list(itertools.zip_longest(*(self.helper.order_drs(list(i)) for _,i in itertools.groupby(drs, lambda s: s.name))))
        drs = []
