...
class Game(Entity):
    def __init__(self):
        ...
        self.model, 'models/custom_cube',
        self.texture = 'textures/rubik_texture'
        self.load_game()
    ...
    def toggle_game_mode(self):
        ...
        msg = dedent(f"{'ACTION mode ON' if self.action_mode else 'VIEW mode ON'} (to switch - press middle mouse button)").strip()
    ...
    def rotate_side_without_animation(self, side_name):
        ...
        for cube in self.CUBES:
            if cube.position in cube_positions:
                ...
                setattr(self.PARENT, f'rotation_{rotation_axis}', 90)
    ...
    def rotate_side(self, side_name):
        ...
        for cube in self.CUBES:
            if cube.position in cube_positions:
                cube.parent = self.PARENT
                self.PARENT.animate_rotation(rotation_axis, 90, duration=self.animation_time)
    ...
    #             if (key == 'mouse1' and collider_name in 'LEFT RIGHT FACE BACK' or key == 'mouse3' and collider_name in 'TOP BOTTOM'):
    #                 self.rotate_side(collider_name)
    #                 break
    #     if key == 'mouse2':
    #         self.toggle_game_mode()
    #     super().input(self, key, is_raw)
